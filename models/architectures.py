...
def get_bbox_from_points(points):
    ...
    x1 = torch.min(points[:, 0]).cpu()
    x2 = torch.max(points[:, 0]).cpu()
    y1 = torch.min(points[:, 1]).cpu()
    y2 = torch.max(points[:, 1]).cpu()
    z1 = torch.min(points[:, 2]).cpu()
    z2 = torch.max(points[:, 2]).cpu()
